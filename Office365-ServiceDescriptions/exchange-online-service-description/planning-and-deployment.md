...
- **仅限云部署** -你的组织在 Exchange Online 中托管了所有用户邮箱。 
    
- **Exchange 混合部署** -你的组织有一些在本地 Exchange 组织中托管的用户邮箱和一些在 Exchange Online 中托管的用户邮箱。 
...
- **IMAP 迁移** -将邮箱数据从基于 IMAP 的电子邮件系统迁移到 Exchange Online。 
    
- 直接转换**exchange 迁移**-在单个直接转换迁移中，将邮箱从 exchange server 2003、exchange server 2007、exchange server 2010、exchange 2013 和托管 exchange 系统迁移到 exchange Online。 
    
- **暂存 Exchange 迁移** -执行暂存迁移，以使用基于 web 的迁移工具从 exchange server 2003 或 exchange server 2007 迁移邮箱，并减少对内部部署基础结构所做的更改。 
    
- **远程移动迁移** -在 exchange 混合部署中将本地 Exchange 邮箱迁移到 exchange Online。 必须拥有 Exchange 混合部署，才能使用远程移动迁移。 
...
- **迁移仪表板** -管理员可以使用 Exchange 管理中心中的迁移仪表板在直接转换或暂存 exchange 迁移中管理到 exchange Online 的邮箱迁移。 管理员还可以使用主控板，将用户邮箱的内容从内部部署 IMAP 服务器迁移到 Exchange Online 的现有邮箱。 主控板为管理员提供了以下功能： 
    
  - **创建并启动多个迁移批处理** -管理员可以创建并排队最多100个迁移批处理。 一次仅能运行一个迁移批次，但是管理员可以排队多个批次，以便在完成一个迁移批次后运行队列的下一个批次。 
    
  - **重启发生故障的迁移批处理** -在迁移批处理的初始同步之后，从本地邮箱将项目复制到迁移批处理中每个用户的云邮箱中，某些邮箱可能无法同步。 管理员可以重新启动该迁移批次，以重试同步失败的邮箱。 
    
  - **获取有关已跳过项目的详细信息** -对于 IMAP 迁移、直接转换迁移和暂存迁移，迁移仪表板将显示跳过的特定项目的相关信息，包括该项目在用户邮箱中的位置和原因。 
    
  - **打开迁移报告** -管理员可以直接从仪表板中打开迁移批处理的迁移统计信息或迁移错误报告。 
    
  - **编辑迁移批处理** -如果暂存 Exchange 迁移或 IMAP 迁移的迁移批处理在迁移队列中，但当前未运行，则管理员可以编辑迁移批处理。 
    
- **Azure Active Directory 同步工具** -Azure Active directory 同步工具在迁移到混合电子邮件方案（同时使用 exchange Online 和内部部署 Exchange 服务器）中扮演重要角色。 该工具将执行从 Active Directory 到 Exchange Online 的单向同步。 完成迁移后，管理员仅需要使用 Exchange Online 就可管理 Active Directory 用户和组。 该工具还为用户提供统一全局地址列表，其中在线环境与内部部署环境持续同步。 
...
- **混合配置向导** -"混合配置" 向导简化了混合部署过程，具体方法是简化功能和服务的内部部署和 Exchange Online 配置。 作为 Exchange Server 2010 Service Pack 2 一部分推出的混合配置向导仅在内部部署组织中运行，同时拥有以下组件： 
...
- **远程 Windows powershell** -作为 Exchange 2006 年12月2011服务更新的一部分，远程 Windows powershell 可用于帮助解决迁移错误。 例如，管理员可以显示迁移批次的诊断信息，以及迁移统计数据和基于主 SMTP 地址为用户显示诊断信息。 
