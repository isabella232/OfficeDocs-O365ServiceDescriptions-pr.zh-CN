...
ms.openlocfilehash: b0e7800c80c0557590e03156a2c30c47eef3f861
...
> [!NOTE]
> <sup>1</sup> 如果使用 Azure Active Directory DirSync，则可以从本地 Active Directory同步到Azure Active Directory的最大通讯组成员数为 15，000。 如果使用的是[Azure AD 连接](/azure/active-directory/hybrid/how-to-connect-sync-endpoint-api-v2)，则该数字为 250，000，但分发组成员的支持限制仍为 100，000。 <br/> 
...
- **按照传输规则重定向邮件** 次数：基于传输规则重定向邮件次数。 例如，Exchange组织 Tailspin Toys 有一个传输规则，用于将发送到 User A 的每条消息重定向到用户 B，用户 B 位于Exchange组织 Contoso 中。 在Exchange组织 Contoso 中，有一个传输规则来将发送到用户 B 的每条消息重定向到用户 C，用户 C 位于Exchange组织 A. Datum Corporation 中。 在这种情况下，邮件将被删除，具有状态代码的未送达报告 （NDR） 并拒绝 *550 5.7.128 传输。规则。拒绝邮件;超过传输规则循环计数，拒绝* 发送到用户 A。使用 X-MS-Exchange-传输规则-循环标题确定通过传输规则重定向邮件的时间。 此页眉还跨越 Exchange 组织边界。
...
- **邮件大小限制**：这是防止大型邮件阻止其他邮件传递并影响所有用户的服务性能所必需的。 这些限制包括一些附件，并且适用于组织范围的所有邮件（入站、出站和内部）。 不会传递大于此限制的消息，并且发件人将收到非传送报告 (NDR) 。 尽管可以向上、向下或按每个用户配置邮件大小限制，但管理员仍可以创建传输规则以限制任何单个附件的最大大小。 若要了解其他信息，请参阅 microsoft [支持较大的电子邮件](https://go.microsoft.com/fwlink/?linkid=2144144)。
...
- **文件附件** 限制 ：电子邮件中允许的最大文件附件数。 即使所有文件附件的总大小未违反消息大小限制，消息中允许的附件数仍有限制。 限制由多部分邮件限制进行控制。
...
- **等待审查的消息过期**：默认情况下，等待审查的消息在两天后过期，管理员无法延长此间隔。 但每 7 天将处理一次已过期的中继邮件。 这意味着，中继邮件会在 2 到 9 天内随时过期。
...
- **接收限制：** 此限制计算 *来自任意和所有源* 的每小时消息数。 这包括来自内部发件人的消息、来自Internet 的消息和来自本地服务器的消息。 超过邮箱的接收限制后，从 Internet 或本地发件人发送到邮箱的邮件将返回到非送达报表中的发件人 (也称为 NDR 或退回邮件) 指出邮箱已超出最大送达阈值。 来自内部发件人的消息将计入限制，但如果超过接收限制，则不会阻止。 一小时后，限制将刷新，邮箱将能够接收邮件。
