...
订阅邮件服务的Exchange Online保留对它们为用户提供的邮件服务的控制。 利用本文档中描述的 Exchange Online 托管计划，电子邮件将托管在同时支持多个客户的服务器上。 这些服务器位于 Microsoft 数据中心内，可供各种设备（从企业网络内部或 Internet）访问。

## <a name="available-plans-for-exchange-online"></a>可用的Exchange Online

Microsoft 365提供了各种计划，以最好地满足组织的需求。 有关允许用户进行订阅的详细信息，Exchange Online完整[订阅比较表](https://go.microsoft.com/fwlink/?linkid=2139145)。
...
- 反垃圾邮件和反恶意软件 [**保护 (通过**](/exchange/antispam-and-antimalware/antispam-and-antimalware)直接访问 Exchange 管理中心管理界面) ：内置的反垃圾邮件和反恶意软件保护 (使用多个反恶意软件引擎扫描入站、出站和内部邮件中的恶意软件) 、自定义反垃圾邮件和反恶意软件策略、隔离 - 管理员管理和隔离 - 最终用户自我管理。
- [**Exchange Online和管理**](/exchange/architecture/client-access/exchange-admin-center)**：** Microsoft 365门户和管理中心访问、Exchange管理中心访问、远程 Windows PowerShell 访问、移动设备的 ActiveSync 策略、[使用率报告](/exchange/monitoring/monitoring)。
- [**高可用性和业务连续性**](/exchange/high-availability/high-availability)：数据中心的邮箱复制、单个项目 (F3 和展台计划) 已删除的邮箱和已删除项目 [恢复](/exchange/recipients/disconnected-mailboxes/restore-deleted-mailboxes)。
- 互操作性、连接性和 [**兼容性**](/exchange/security-and-compliance/interoperability-connectivity-and-compatiblity)：OWA 和 Outlook 中的 Skype for Business 状态、SharePoint 互操作性、EWS 连接支持 (EWS 应用支持（应用于模拟) 、SMTP 中继支持）。
...
- 报告 [**功能和**](/office365/servicedescriptions/exchange-online-service-description/reporting-features-and-troubleshooting-tools)疑难解答工具 **：Microsoft 365 管理中心报告**、Excel报告工作簿、Web 服务报告、统一消息报告 (E3/E5 和仅计划 2) 。 邮件跟踪和[审核](/exchange/exchange-auditing-reports-exchange-2013-help)报告可通过直接访问 EAC Exchange管理中心 (EAC) 访问。
...
- 共享 [**和**](/exchange/sharing/sharing)协作：必须包含和部署联合共享 (包括日历发布) 、网站邮箱 (SharePoint Online) 以及公用文件夹 (不适用于 F3 和展台计划) 。
...
- **Exchange中心的** 新增功能：有关 Exchange 管理中心中的新功能的信息，请参阅 Exchange [管理中心的新增功能](/exchange/whats-new)。
