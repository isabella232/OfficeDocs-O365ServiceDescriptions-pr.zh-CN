...
- Adm_ServiceDesc_top
ms.assetid: ''
description: Project是 Microsoft 最新的基于云的工作和项目管理产品/服务。 Project Web 服务提供了简单、强大的工作管理功能，以满足大多数需要和角色。
...
| | 路线图 | 仅查看 | 仅查看 | 是 | 是 |
| **Reporting3**<sup></sup> | 从 Web Project读取报告 | 是 | 是 | 是 | 是 |
...
| **Power Automate工作流和业务流程**<sup>流6</sup> | 定义和使用Power Automate数据（包括自定义表Project列）的云流 | 否 | 是 | 是 | 是 |
| | 定义Power Automate数据（包括自定义表Project列）的业务流程流。 | 否 | 否 | 是 | 是 |
| | 使用Power Automate数据的业务流程流Project流 | 否 | 是 | 是 | 是 |
| **安全性和用户管理** | Office新式组 | 是 | 是 | 是 | 是 |
| | 第三方应用 | 否 | 是 | 是 | 是 |
| | 连接 *Project第三方* 应用为 Web 创建应用程序 <sup>4</sup> | 仅查看<sup>7</sup> | 是 | 是 | 是 |
...
<sup>4</sup> 构建和使用Power Apps Web 或 Project Online Project 数据的开箱即用 Project 应用程序之外的单独 Project Online 需要单独的 Power Apps 订阅。 <br/>
...
<sup>6</sup> Power Automate内部Project仅限于应用程序Project上下文。 这意味着，对于触发器和操作，包含在流Project可以：
   ...
   - 通过内置触发器/操作Project在 (应用程序内直接) 。

<sup>7</sup> 仅从 Power App 内授予仅查看权利。<br/>
<sup>*</sup>Office 365用户可以更新一组有限的字段， (标记为已完成，进度) 分配给他们的任务。
...
Project路线图要求使用 Power Automate，该指南作为你的 Project 订阅的一部分进行预配。 Power Automate Web Project随附的 Flow 功能显示为 Microsoft 365 管理中心 的"Project 应用程序"部分中的 Project  服务Microsoft 365 管理中心。

#### <a name="project-for-the-web-and-microsoft-dataverse"></a>Project和 Microsoft Dataverse 的搜索

Project要求使用 Microsoft Dataverse 来存储其数据。 Microsoft Dataverse 数据库将预配为订阅的Project一部分。 对 Microsoft Dataverse 功能的访问权限仅限于存储和访问数据，以支持Project Web 服务。 Project所需的 Microsoft Dataverse 功能Project应用程序的"应用程序"部分中的 Common **Data Service for** Microsoft 365 管理中心。
...
除了授权 Dataverse 存储容量Project还有额外的 Microsoft 订阅。 有关其他 Dataverse 存储容量权利，请参阅动态  [365](https://go.microsoft.com/fwlink/?LinkId=866544&clcid=0x409)  许可指南和Power Apps、Power Automate  [Power Virtual Agents指南](https://go.microsoft.com/fwlink/?LinkId=2085130&clcid=0x409)。
...
若要了解有关 Web Project中数据加密的信息，请参阅  [Microsoft Dynamics 365 中的](/microsoft-365/compliance/office-365-encryption-in-microsoft-dynamics-365)加密。
...
若要及时了解即将进行的更改，包括新功能和已更改的功能、计划的维护或其他重要通知，请访问消息中心。 有关详细信息，请参阅[消息中心](/microsoft-365/admin/manage/)。
