...
Microsoft Project 可通过三种不同的订阅使用，以最大限度地满足组织的需求。 这些订阅是项目计划1、项目计划3和项目计划5。 若要在各个订阅之间比较功能，请参阅[跨应用程序和订阅的功能可用性](#feature-availability-across-applications-and-subscriptions)。
...
在上表中，检查次数（**x**）指示计划提供的常规级别的功能。 例如，对于 Project Online，计划5提供更多功能，然后计划3。
...
本部分中列出的功能仅与 web 数据存储的项目中的数据交互。 所有这些功能均可用于所有 Microsoft Project 订阅： Project Plan 1、Project Plan 3 和 Project Plan 5。
...
|**报告**<sup>3</sup> ||||||
|开箱即用报告 | 使用预建的报告来了解项目、项目群、项目组合和资源数据。 | 否 | 可访问 | 是 | 是 |
|自定义报告 | 通过基于现成报告或从头开始构建自定义报告，了解对您的组织至关重要的数据。 | 否 | 可访问 | 是 | 是 |
|仪表板和门户 | 构建自定义仪表板和门户，以帮助满足特定的组织需求。| 否 | 可访问 | 是 | 是 |
|**可用性** ||||||
|共同创作 | 与风险承担者和工作组成员共同构建、编辑和更新任务列表、项目日程等。| 仅查看 | 是 | 是 | 是 |
|图形标记 | 使用图形标记了解任务状态、工作分配等。| 仅查看 | 是 | 是 | 是 |
|**移动性** ||||||
|移动应用程序 | 使用 PowerApps 生成自定义移动应用程序。<sup>4</sup> |否 | 否 | 可访问 | 是 |
|**自定义和集成**||||||
|PowerApps | 在 PowerApps 中构建和使用适用于 web 和移动解决方案的解决方案。<sup>4</sup> | 否 | 否 | 可访问 | 是 |
|工作流 | 自动化并将业务流程与电源自动化相集成。<sup>5</sup> | 否 | 否 | 可访问 | 是 |
...
|Notifications | 使用 Outlook 和团队通知保持任务分配、状态和重要项目事件的最上面。<sup>1</sup> | 是 | 是 |
