...
ms.locfileid: "58702304"
---
# <a name="exchange-online-for-us-government-environments"></a>Exchange Online美国政府环境

本文概述了美国政府云与商业云之间的功能差异，如 Exchange Online [service description 中所列](../../exchange-online-service-description/exchange-online-service-description.md)。 Exchange Online DoD 政府社区云 (GCC) 、GCC、美国国防部 (DoD) 可用。
...
下表概述了指定的 Exchange Online 功能在 GCC、GCC 高和 DoD 环境中是否可用。 当支持声明存在差异或缺少 (时) ，则提供其他上下文。<br><br>

| 功能 | GCC | GCC 高 | DoD | 主要注意事项 |
|:-----|:-----|:-----|:-----|:-----|
|**[规划和部署](../../exchange-online-service-description/planning-and-deployment.md)**|||||
|支持的混合部署|是|是|是|为了与本地Exchange Server共存，Microsoft 需要安装至少一个 Exchange Server 2013 客户端访问服务器 (或 Exchange Server 2016.) 。 Exchange Server 2010 及更早版本不受支持。|
|支持的 IMAP 迁移|是|是|是||
|支持的直接转换迁移|是|是|是||
...
|隔离 - 管理员管理|是|是|是||
|隔离 - 最终用户自我管理|是|是|是||
|Microsoft Defender for Office 365|是|是|是|需要 G5 服务 (或购买加载项) 。<br><br>用户和域模拟以及欺骗智能的防钓鱼功能在高GCC DoD 中尚不可用。|
...
高GCC DoD 环境都是需要明确审批和配置出站连接的受限环境。 此外，Microsoft 无法容纳允许从这些环境出站访问商业云服务 (商业 Office 365、Google GSuite、Amazon Web Services 等) 。
...
- 将加密电子邮件从 GCC High 或 DoD 发送到相同环境中收件人时：
    ...
    - 发件人可以在适用于 PC 和 Mac Outlook Outlook 网页版 的 Outlook 中手动加密电子邮件，或者组织可以设置策略以使用 Exchange 规则加密电子邮件。
    
    - GCC/DoD 内的收件人在 Outlook for PC 和 Mac 和 Outlook 网页版 接收与所有其他 Office 365相同的内联阅读体验。

<!-- end list -->

- 将加密电子邮件从 GCC High 发送到该环境外部的收件人 (DoD、GCC 和商业) ：

    - GCC High 内的发件人可以在高GCC发送加密电子邮件。
...
-  (High 内的租户、GCC 和商业云中的租户之间以及通过混合共存 GCC (Exchange 2013 或更高版本) 支持联合身份验证信任) 包括双向忙/闲共享) 。
...
Outlook适用于 iOS 和 Android 的 GCC 也适用于高和 DoD 环境。 若要详细了解这些环境中的功能限制和管理，请参阅在 Outlook 中为[iOS](/exchange/clients-and-mobile-in-exchange-online/outlook-for-ios-and-android/outlook-for-ios-and-android-in-the-government-cloud)和 Android 政府社区云。
