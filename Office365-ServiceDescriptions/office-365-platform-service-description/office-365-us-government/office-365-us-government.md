...
description: 为了响应美国公共部门不断变化的独特要求，Microsoft 已创建一个或多个Office 365 US 政府计划 (Office 365 政府版) 。 本文概述了特定于美国环境Office 365 政府版的功能。
...
为了响应美国公共部门不断变化的独特要求，Microsoft 已创建Office 365 政府版计划 (或Office 365 政府版) 。 此服务说明概述了特定于美国环境Office 365 政府版功能。 建议您与其他服务说明一起阅读此服务Microsoft 365 [Office 365说明](../../office-365-service-descriptions-technet-library.md)。
...
- **Office 365 GCC 环境** 符合联邦云服务要求，包括 FedRAMP High、国防联邦购置条例补充程序 (DFARS) 以及针对刑事犯罪和联邦税务信息系统 (CJI 和 FTI 数据类型) 的要求。
...
尽管资格条件在产品/服务Office 365 政府版一致，但 Microsoft 将仅同意适用于高GCC ITAR 合同语言。
...
- Microsoft Planner 计划
- 表单数据
- PowerApps/PowerAutomate 数据

## <a name="office-365-government-and-third-party-services"></a>Office 365 政府版和第三方服务

Office 365可以将第三方应用程序集成到 SharePoint Online 网站、Teams、Office Microsoft 365 企业应用版 (应用程序（如 Word、Excel、PowerPoint 和Outlook) 和 Outlook Web App。 此外，Office 365 还支持与第三方服务提供商集成。 这些第三方应用程序和服务可能参与存储、传输和处理组织在 Office 365 基础结构外部的第三方系统上的客户数据，因此未涵盖在 Office 365 合规性和数据保护协议中。 我们建议您在为组织评估这些服务的相应使用情况时，查看第三方提供的隐私和合规性声明。
...
通过 FastTrack Center Benefit for Office 365 <sup>1</sup>，你可以与 FastTrack 专家远程合作，让 Office 365 环境可供使用，并计划在组织中部署和使用。 FastTrack 流程提供了载入和用户采用服务。
...
可以在此处找到FastTrack中心过程[的特定信息](/FastTrack/us-gov-appendix-overview)。 有关参与角色和职责的细目，FastTrack[责任](/FastTrack/us-gov-appendix-fasttrack-responsibilities)和[职责](/FastTrack/us-gov-appendix-your-responsibilities)。
...
选择迁移[FastTrack](https://fasttrack.microsoft.com/)需要向管理其数据迁移的团队授予访问权限。 这些人员是美国政府公民，在针对美国政府服务客户Office 365以下背景检查。<br><br>
...
有关 Office 365 Azure ExpressRoute 支持的详细信息，请参阅 [Office 365 的 Azure ExpressRoute ](/microsoft-365/enterprise/azure-expressroute)。
...
有关安全与合规中心&amp;的信息以及指向其他信息和可用性的链接，请参阅Microsoft 365[合规性指南&amp;](/office365/servicedescriptions/microsoft-365-service-descriptions/microsoft-365-tenantlevel-services-licensing-guidance/microsoft-365-security-compliance-licensing-guidance)。
...
<br/> <sup>2</sup> Yammer Enterprise美国政府版的组件，Office 365许可的每位用户作为独立产品/服务在 GCC 中Office 365购买。 This offer is currently limited to customers that purchase Office 365 GCC under Enterprise Agreements and Enterprise Subscription Agreements. Yammer在高GCC DoD 中不可用。
<br/><sup>3</sup> 通话套餐是加载项， (添加语言或自带中继) 。
<br/><sup>4</sup> 不包含在内，但可以单独购买加载项。 Visio包括桌面Visio作为订阅的一部分。
...
|---|---|---|---|---|
|**Office 365 管理**|||||
|使用Microsoft 365 管理中心管理Office 365|是<sup>14</sup>|是<sup>14</sup>|是|是<sup>14</sup>|
|从 Office 365 中管理核心服务设置|是|是|是|是|
|使用 Windows PowerShell 管理 Office 365|是|是|是|是|
|使用 Azure 信息保护保护内容|否<sup>1</sup>|是<sup>13</sup>|是<sup>13</sup>|是<sup>13</sup>|
...
|Microsoft Bookings|否|是<sup>19</sup>|是<sup>19</sup>|否|
|Microsoft 简报电子邮件|否|否|否|否|
|Microsoft Power Automate|是|是|是|是|
...
|什么是 Microsoft Stream（经典）？|是<sup>17、22</sup>|是<sup>17、22</sup>|是<sup>17、22</sup>|是<sup>17、22、18</sup>|
...
|Office Delve|是<sup>15</sup>|是<sup>15</sup>|是|是<sup>15</sup>|
...
|云标识、联合身份或多重身份验证|是|是|是|是|
|Office 365 桌面安装程序|是|是|是|是|
|使用 Office 365 管理用户|是|是|是|是|
|使用 .csv 文件批量上载|是<sup>7</sup>|是<sup>7</sup>|是<sup>7</sup>|是<sup>7</sup>|
|目录同步工具|是|是|是|是|
|Exchange 简单（直接转换）迁移|是|是|是|是|
|删除帐户并重置用户密码Microsoft 365或通过使用 Windows PowerShell <sup>3</sup>|是|是|是|是|
|用户可以更改自己的密码<sup>4</sup>|是|是|是|是|
|管理许可证|是<sup>5、6</sup>|是<sup>5、6</sup>|是<sup>5、6</sup>|是<sup>5、6</sup>|
|从 Office 365 管理安全组|是|是|是|是|
|多个管理员角色可用|是|是|是|是|
|允许合作伙伴为您管理 Office 365|是<sup>9</sup>|是<sup>9</sup>|是<sup>9</sup>|是<sup>9</sup>|
...
|“服务运行状况”或“服务状态”页面中显示状态信息|是|是|是|是|
|Microsoft 365 管理中心仪表板中显示的单个警报的状态|是|是|是|是|
|是|是|是|是|是|
...
|[活动报告](/microsoft-365/admin/activity-reports/activity-reports#which-activity-reports-are-available-in-the-admin-center)|是|是|是|是|
|[Microsoft Graph 使用情况报告 API](/graph/api/resources/report)|是|是|是|是|
|[Microsoft Graph API (BETA) ](/graph/api/resources/report?view=graph-rest-beta&preserve-view=true)|是|是|是|是|
...
|通知发送给 消息中心（当需要操作时）|是|是|是|是|
|针对某些服务更新的 Roadmap.office.com|否<sup>8</sup>|否<sup>8</sup>|否<sup>8</sup>|否<sup>8</sup>|
|打开定向发布的选项|是<sup>8、22</sup>|是<sup>8、22</sup>|是<sup>8、22</sup>|是<sup>8、22</sup>|
...
|云应用安全|否<sup>10、17</sup>|否<sup>10、17</sup>|是<sup>17</sup>|否<sup>10、17</sup>|
|Microsoft Defender for Office 365|否<sup>10、16 </sup>|否<sup>10、16 </sup>|是<sup>16</sup>|否<sup>10、16</sup>|
|客户锁箱|否<sup>10</sup>|否<sup>10</sup>|是|否<sup>10</sup>|
|客户密钥 <sup>20</sup> |否|否|是|否|
|高级电子数据展示|否<sup>10</sup>|否<sup>10</sup>|是|否<sup>10</sup>|
|基本审核<sup>21</sup>|是|是|是|是|
|高级审核<sup>21</sup>|否|否|是|否|
|Microsoft 安全分数<sup>14</sup>|是<sup>7、13</sup>|是<sup>7、13</sup>|是<sup>7、13</sup>|是<sup>7、13</sup>|
|威胁智能|否<sup>10</sup>|否<sup>10</sup>|是|否<sup>10</sup>|
...
|是|否<sup>9</sup>|否<sup>9</sup>|否<sup>9</sup>|否<sup>9</sup>|
|提供委派管理|否<sup>9</sup>|否<sup>9</sup>|否<sup>9</sup>|否<sup>9</sup>|
...
> <sup>1</sup> Azure 信息保护不包括在内，但可以单独购买加载项，并启用支持的信息权限管理 (IRM) 功能。 一些 Azure 信息保护功能需要订阅 Microsoft 365 企业应用版，但此订阅不包含在Office 365 政府版 G1。
...
<br/><sup>4</sup> 若要了解如何为用户设置自助服务密码管理策略，请参阅使用安全信息重置 [工作或学校密码](/azure/active-directory/user-help/active-directory-passwords-update-your-own-password)。
<br/><sup>5</sup> Reducing seats that were purchased with a term discount may be subject to an early termination fee. 这不适用于按月支付的订阅。
<br/><sup>6</sup> 以下计划不支持更改许可证的许可证席位Microsoft 365 管理中心：Office 365 政府版 G1、Office 365 政府版 G3、Office 365 政府版 F3。
<br/><sup>7</sup> 尚未在 GCC 中可用，但即将推出。
<br/><sup>8</sup> 对于 Office 365 政府版 G1、G3 和 F3，Office 365商业版路线图适用;但是，由于合规性要求，特定服务更新可能有一些[差异或延迟](https://www.microsoft.com/trust-center)。
<br/><sup>9</sup> 尚不可用于Office 365 政府版产品/服务，但即将推出。
<br/><sup>10</sup> 不包含在内，但可以在外接程序中作为单独的GCC。
<br/><sup>11</sup> 不支持Office 365 政府版产品/服务。
<br/><sup>12</sup> 在 中可用 [https://securescore.office.com](https://securescore.office.com)。 必须拥有管理员权限。 有关详细信息，请参阅 [Microsoft 安全分数](/microsoft-365/security/mtp/microsoft-secure-score)。
<br/><sup>13</sup> 尚不可用于 DoD 环境，但即将推出。
<br/><sup>14</sup> 管理中心不包括 DoD 或高GCC使用情况分析。
<br/><sup>15</sup> 不支持高GCC DoD 环境。
<br/><sup>16</sup> High 和 DoD 中尚未提供适用于用户和域模拟和欺骗智能GCC防钓鱼。
<br/><sup>17</sup> 尚未在GCC中可用，但即将推出。
<br/><sup>18</sup> 仅适用于 Microsoft Stream：不发布或共享。
<br/><sup>19</sup> 不适用于 Microsoft Graph API 或 Microsoft Teams。
<br/><sup>20</sup> 有关详细信息，请参阅使用客户密钥 [的服务加密](https://support.office.com/article/f2cd475a-e592-46cf-80a3-1bfb0fa17697)。
<br/><sup>21</sup> Microsoft 365中的审核使组织能够查看组织中跨不同服务的多种审核Microsoft 365。 使用 [安全&合规审核日志](/office365/servicedescriptions/office-365-platform-service-description/office-365-securitycompliance-center) 搜索来查看组织的用户和管理员活动。 您还可以使用[Office 365管理活动API](/office/office-365-management-api/office-365-management-activity-api-reference)从统一审核日志中检索事件。 用户或管理员执行经审核的活动时，将生成审核记录并存储在客户组织的审核日志中。 审核记录在 审核日志)  (中保留和搜索的时间长度取决于客户的 Office 365 或 Microsoft 365 企业版 订阅以及分配给特定用户的许可证类型。
...
|Microsoft <sup>Forms7</sup>|是|是|是|否|
...
|[Office Mobile for iPad/iPhone](../../office-applications-service-description/office-applications.md#office-mobile-for-ipadiphone)|是|是|是|是|
|[Office Mobile for Android](../../office-applications-service-description/office-applications.md#office-mobile-for-android)|是|是|是|是|
...
|Office Mobile for Windows 10 tablets|是|是|是|是|
|Outlook iOS 和 <sup>Android5，4</sup>|是|是|是|是|
...
|改进了与 Power BI (Power BI 数据类型的集成，Power BI数据集) <sup>3</sup>||否|否|否|
|智能数字墨迹||否|否|否|
|Power View (适用于 2016/2019 永久，因为 O365 和 2021 永久策略中将删除所有客户) ||否|否|否|
|智能查找|是|否|否|否|
...
|外部共享<sup>4</sup>||是|否|否|
|插入图片||是<sup>5</sup>|是<sup>5</sup>|是<sup>5</sup>|
...
|[**Microsoft PowerPoint**](../../office-applications-service-description/office-applications.md#microsoft-powerpoint)在政府云中完全可用，但以下功能除外，这些功能并非在所有政府云中可用，如表所示。||**GCC**|**GCC 高级**|**DOD**|
...
|重复使用幻灯片||否|否|否|
|版本历史记录|是|否|否|否|
|**Microsoft Whiteboard** 云中的云解决方案目前仅在中心客户端上可用，桌面版不可用。||**GCC** <sup>2</sup>|**GCC High2**<sup></sup>|**DOD2**<sup></sup>|
...
|实时状态|是|否|否|否|
|对内容的反应|是|否|否|否|
|[**Microsoft Word**](../../office-applications-service-description/office-applications.md#microsoft-word)在政府云中完全可用，以下功能除外，并非所有政府云中都提供这些功能，如表所示。||**GCC**|**GCC 高级**|**DOD**|
|3D 模型||否|否|否|
|3D 嵌入式动画||否|否|否|
|与合著者进行上下文聊天：与文档中共同创作者聊天||否|否|否|
|语音听写|是|是|是|否<sup>1</sup>|
...
|智能查找|是|否|否|否|
|点击||否|否|否|
|转换||是<sup>6</sup>|是<sup>6</sup>|是<sup>6</sup>|
|版本历史记录|是|否|否|否|

> <sup>1</sup> 即将上市。
<br/><sup>2</sup> 未登录Surface Hub (本地) 。
<br/><sup>3</sup> Excel/Power BI云连接功能仅受 Power BI 支持的云环境支持。
<br/><sup>4</sup> 外部共享适用于 GCC 环境，对 GCC 高和 DOD 环境禁用 (您的组织中的用户可以填写表单并提交响应、复制和共享表单作为模板、[共同](https://support.office.com/article/d5bb5cf0-8401-4c15-bb8c-8e108cd7e69b)创作或协作处理表单以及访问表单[结果) 。](https://support.office.com/article/02859424-341d-406f-b32a-9a0fbaf357af) [](https://support.office.com/article/82ea9d8a-260a-47a0-afdb-497f3d746e3f) 详细了解如何 [为组织关闭或打开 Microsoft Forms](https://support.office.com/article/cc52287a-4550-464d-9a1b-457bf9df2240#PickTab=Configure) 。
<br/><sup>5</sup> 已针对 必应/GCCH/DOD 环境禁用GCC图像搜索"功能插入图像。
<br/><sup>6</sup> Word，Excel PowerPoint Windows客户端，而非 Web、MacOS、iOS 或 Android。
