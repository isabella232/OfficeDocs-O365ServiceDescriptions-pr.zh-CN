...
## <a name="security-amp-compliance-center-availability-for-business-and-enterprise-plans"></a>适用于 &amp; 业务和企业计划的安全合规中心可用性
...
|[Exchange Online、SharePoint Online](https://docs.microsoft.com/microsoft-365/compliance/data-loss-prevention-policies)和 OneDrive for Business<sup>2</sup>的数据丢失防护 | 否 | 否  |是   | 是 | 是 | 是 | 否  |
...
|[电子数据展示保留 (基于查询的电子数据展示保留) ](https://docs.microsoft.com/office365/securitycompliance/ediscovery-cases#step-4-place-content-locations-on-hold)  |否   |否   |否  |否   |是   |是   |否   |
...
<sup>1</sup> 包含基本审核计划的所有计划的审核日志 (E5) 除外）保留 90 天。 由于 E5 包含高级审核，因此审核日志最多保留一年。 此外，您还可以使用 [Office 365 管理活动 API](https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference) 从统一管理审核日志。

<sup>2</sup> 需要 Office 365 数据丢失防护加载项。

Azure 信息保护 P1 和 P2 中还包括<sup>3</sup>个敏感度标签。
...
|[电子数据展示保留 (基于查询的电子数据展示保留) ](https://support.office.com/article/eDiscovery-cases-in-the-Office-365-Security-Compliance-Center-8dd335ab-29d0-41c3-8dd8-9f7c7481e60c#step3_1)  |否   |是   |否   |否   |是   |否  |是<sup>10</sup> |否<sup>4</sup>  |否<sup>4</sup>  |
...
<sup>1</sup> 利用信息治理，用户可以创建、发布标签并手动将标签应用于文档;使用驱动器寄送或网络导入数据。 这些功能在 E3 和 E5 中可用，仅在 E1 中提供有限的可用性。 有关 E1、E3 和 E5 中可用功能的完整列表，请参阅详细的 Microsoft 365 合规性许可比较。 [ (PDF) ](https://docs.microsoft.com/office365/servicedescriptions/downloads/microsoft-365-compliance-licensing-comparison.pdf)  | [ (Excel) ](https://docs.microsoft.com/office365/servicedescriptions/downloads/microsoft-365-compliance-licensing-comparison.xlsx)

<sup>2</sup> 需要购买Exchange Online Archiving加载项。

<sup>3</sup> 高级信息治理允许您保留重要信息，并基于保留或删除策略或同时对信息进行分类来删除不重要的信息。 它包括智能/自动操作，如建议策略、自动将标签应用于数据、基于敏感数据类型或查询应用标签、处置评审以及智能导入筛选器的使用。 它还包括用于出于安全性和合规性目的查看员工通信的监督功能。
...
<sup>8</sup> 包含基本审核的所有计划的审核日志将保留 90 天。 此外，您还可以使用 [Office 365 管理活动 API](https://docs.microsoft.com/office/office-365-management-api/office-365-management-activity-api-reference) 从统一管理审核日志。

<sup>9</sup> 需要 Office 365 数据丢失防护加载项。

<sup>10</sup> 仅限于存储在 OneDrive for Business 中的文件。

<sup>11</sup> 需要 Exchange Online 计划 2 或Exchange Online Archiving许可证才能使用保留策略将用户邮箱置于保留状态。
...
合规中心在由世纪网运营的 Office 365 的 E3 计划中提供。
