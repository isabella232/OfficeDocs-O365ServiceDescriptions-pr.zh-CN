...
ms.locfileid: "60649231"
---
# <a name="microsoft-365-guidance-for-security-amp-compliance"></a>Microsoft 365 &amp; 合规性指南

对于本文而言，租户级服务是一种在线服务，当为租户 (独立版或 Office 365 或 Microsoft 365 计划的任何用户购买时，会为租户中的所有用户部分或完整激活) 计划。 &mdash; &mdash; 尽管某些未授权的用户可能从技术上能够访问该服务，但任何打算从该服务受益的用户都需要许可证。
...
Microsoft 365 中的高级审核为用户和管理员活动提供一年审核日志保留期，并提供了创建自定义 审核日志 保留策略来管理其他 Microsoft 365 服务的 审核日志 保留。 它还提供对关键事件的访问权限，以进行调查，以及访问 Office 365 活动 API。 有关详细信息，请参阅高级[审核Microsoft 365。](/microsoft-365/compliance/advanced-audit)
...
Office 365 E5/A5/G5、Microsoft 365 E5/A5/G5、Microsoft 365 E5/A5/G5 合规性、Microsoft 365 F5 安全 & 合规性和 Microsoft 365 E5/A5/G5 内部风险管理为用户提供了从通信合规性中获益的权利。
...
当前已注册 FSI 合规性计划的客户将需要购买新的 Microsoft 云合规性计划的订阅。 有关详细信息 [，请参阅](https://aka.ms/cpmc)。
...
默认情况下，Microsoft 云合规性计划在租户级别为从该服务受益的所有用户启用。 有关详细信息 [，请参阅](https://aka.ms/cpmc)。
...
电子数据展示为公司内的 IT 和法律部门提供调查和电子数据展示解决方案，以在导出出 Microsoft 365 系统之前识别、收集、保留、减少和查看与调查或诉讼相关的内容。
...
默认情况下，Teams消息和频道消息是租户中所有用户 *(* 启用) DLP 功能的启用位置和工作负载。 有关使用 DLP 策略的信息，请参阅 [数据丢失防护概述](/office365/securitycompliance/data-loss-prevention-policies)。
...
借助 Office 365 数据丢失防护 (DLP) for Exchange Online、SharePoint Online 和 OneDrive for Business (，组织可以识别、监视和自动保护电子邮件和文件（包括存储在 Microsoft Teams 文件存储库) 中的文件）中的敏感信息。
...
Microsoft 365 E3/A3/Business 高级版、Office 365 E3/A3 以及 Office 365 数据丢失防护和 F5 合规性与 F5 安全 & 合规性为用户提供了从 Office 365 DLP Exchange Online、SharePoint Online 和 OneDrive for Business 受益的权利。

### <a name="how-is-the-service-provisioneddeployed"></a>如何设置/部署服务？

默认情况下，Exchange Online、SharePoint网站和 OneDrive 帐户 (租户中所有用户) DLP 功能的工作负荷启用位置。 有关使用 DLP 策略的信息，请参阅 [数据丢失防护概述](/microsoft-365/compliance/data-loss-prevention-policies)。
...
若要将加密密钥分配给授权用户的 Office 365 和/或 Microsoft 365 组织内的数据，请按照双密钥加密部署说明操作。
...
Microsoft Defender for Cloud Apps 是 CASB (解决方案中的云访问安全代理) ，让客户能够灵活地实现核心功能并支持多种类型的部署。 Microsoft Defender for Cloud Apps 是基于用户的订阅服务。 每个许可证是每个用户每月的许可证，可以授权为独立产品或作为多个许可计划的一部分，如下所示。
...
若要从适用于云应用的 Defender 中的条件访问应用控制功能中获益，用户还必须获得 Azure Active Directory P1 的许可，其包含在 企业移动性 + 安全性 F1/F3/E3/A3/G3、企业移动性 + 安全性 E5、Microsoft 365 E3/A3/G3、Microsoft 365 E5/A5/G5 以及 Microsoft 365 E5/A5/G5/F5 安全与 Microsoft 365 F5 安全合规性中。 &amp;
...
Microsoft Defender for Endpoint P1 作为独立用户订阅许可证提供给商业和教育客户。 2022 年第一季度，Defender for Endpoint P1 还将作为 Microsoft 365 E3/A3 的一部分提供。
...
## <a name="microsoft-graph-apis-for-teams-data-loss-prevention-dlp-and-for-teams-export"></a>Microsoft Graph) DLP Teams (和 Teams 导出的 Microsoft Teams API
...
[数据丢失防护 (DLP) ](/microsoft-365/compliance/dlp-microsoft-teams)功能在组织中广泛使用Microsoft Teams，尤其是在组织转移到远程工作时。 如果您的组织具有 DLP，则现在您可以定义防止用户共享频道或聊天会话中Microsoft Teams敏感信息的策略。

信息保护和管理功能在组织中广泛使用Microsoft Teams，尤其是在组织转移到远程工作时。 使用[Teams API，](/microsoftteams/export-teams-content)可以将数据导出到第三方电子数据展示或合规性存档应用程序，以确保符合合规性要求。
...
有关种子设定容量和消耗费用的信息，请参阅Graph聊天消息[的一些要求](/graph/teams-licenses)。
...
隐私管理内置于租户Microsoft 365 合规中心租户[](https://compliance.microsoft.com/homepage)内所有用户在租户级别启用。 我们建议为打算从该服务受益并保护的任何用户获取许可证。
...
- 利用对 API 的编程访问。
- 通过集成与其他利益干系人Microsoft Teams， (需要适当的许可证才能进行Microsoft Teams) 。</br>
...
Microsoft 365 E5/A5/G5、Microsoft 365 E5/A5/G5/F5 合规性和 F5 安全 & 合规性、Microsoft 365 信息保护和管理 E5/A5/G5 以及 Office 365 E5/A5/G5 为用户提供了从记录管理中获益的权利，包括将项目声明为记录或监管记录、自动应用保留或记录标签并执行处置评审 (根据可训练分类器自动应用保留标签) 。
